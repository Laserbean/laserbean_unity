...
namespace Dossamer.PanZoom
{
    ...
    public class PanZoomBehavior : MonoBehaviour
    {
        [Header("Input System")]
        [SerializeField]
        protected bool useNewInputSystem = true;

        // track previous frame states for touch/mouse transitions
        protected int previousTouchCount = 0;
        ...
        protected virtual void CheckIsPanning()
        {
            ...
            if (GetAxesOfType(InputMethod.HorizontalAxis).Count > 0 && GetHorizontalAxis() != 0f)
            {
                isInputMethodActive[InputMethod.HorizontalAxis] = true;
            }

            if (GetAxesOfType(InputMethod.VerticalAxis).Count > 0 && GetVerticalAxis() != 0f)
            {
                isInputMethodActive[InputMethod.VerticalAxis] = true;
            }

            if (GetAxesOfType(InputMethod.PointerHorizontal).Count > 0 && IsPointerPressed(mouseButton) && GetTouchCount() <= 1)
            {
                isInputMethodActive[InputMethod.PointerHorizontal] = true;
            }

            if (GetAxesOfType(InputMethod.PointerVertical).Count > 0 && IsPointerPressed(mouseButton) && GetTouchCount() <= 1)
            {
                isInputMethodActive[InputMethod.PointerVertical] = true;
            }

            if (GetAxesOfType(InputMethod.Scrollwheel).Count > 0 && GetMouseScrollDelta().y != 0) {
            ...
            if (customAxes.Count > 0) {
                foreach (AxisData axis in customAxes)
                {
                    ...
                    {
                        if (GetCustomAxis(axis.customInputMethod) != 0)
                        {
                    ...
                    {
                        if (GetCustomAxis(axis.customSecondaryInputMethod) != 0)
                        {
                            isInputMethodActive[InputMethod.CustomAxis] = true;
                        }
                    }
            ...
            if (IsPanning)
            {
                ...
                if (WasPointerPressedThisFrame(mouseButton))
                {
                    mousePanStartPosition = GetPointerPosition();
                    lastMouseDelta = null; 
                }

                // if touch count just increased to >= 2, reset pinch tracking
                if (GetTouchCount() >= 2 && previousTouchCount < 2)
                {
            ...
            previousTouchCount = GetTouchCount();
        }
            ...
            foreach (AxisData axis in axes.Values)
            {
                if (axis.IsEnabled)
                {
                    ...
                    if (axis.IsSecondaryInputEnabled)
                    {
                        ...
                        if (!axis.DoesSecondaryNeedTrigger || GetButton(axis.secondaryInputTrigger))
                        {
        ...
        protected virtual void UpdatePosition()
        {
            if (IsPanning)
            {
                    ...
                    isInputMethodActive[InputMethod.CustomAxis])
                {
                    ...
                    {
                        float magnitude = GetHorizontalAxis() * inputMultipliers[axis.inputMethod];
                        AddAxisToContribution(axis, ref contribution, magnitude);
                    }

                    foreach (AxisData axis in verticalAxes)
                    {
                        float magnitude = GetVerticalAxis() * inputMultipliers[axis.inputMethod];
                        AddAxisToContribution(axis, ref contribution, magnitude);
                    }

                    foreach (AxisData axis in scrollwheelAxes)
                    {
                        float magnitude = GetMouseScrollDelta().y * inputMultipliers[axis.inputMethod];
                        AddAxisToContribution(axis, ref contribution, magnitude);
                    }
                    ...
                    if (isInputMethodActive[InputMethod.CustomAxis])
                    {
                        foreach (AxisData axis in customAxes)
                        {
                            ...
                            float magnitude = GetCustomAxis(axisString) * inputMultipliers[axis.inputMethod];
                            AddAxisToContribution(axis, ref contribution, magnitude);
                        }
                    }
                ...
                {
                    Vector3 mouseDelta = GetPointerPosition() - mousePanStartPosition;
        ...
        {
            // Prefer new Input System's Touchscreen if available
            if (useNewInputSystem && Touchscreen.current != null)
            {
                ...
                {
                    if (t.press != null && t.press.isPressed)
                    {
                        activePositions.Add(t.position.ReadValue());
                    }
                }

                if (activePositions.Count >= 2)
                {
                    float absoluteDistance = Vector2.Distance(activePositions[0], activePositions[1]);
                    float proportionalDistance = absoluteDistance / new Vector2(Screen.width, Screen.height).magnitude;
            ...
            if (Input.touchCount >= 2)
            {
                ...
                float absoluteDistance = Vector2.Distance(touch0, touch1);
                float proportionalDistance = absoluteDistance / new Vector2(Screen.width, Screen.height).magnitude;
        ...
        // --- New Input System helper wrappers ---
        ...
        protected bool IsPointerPressed(MouseButton button)
        {
            if (useNewInputSystem && Mouse.current != null)
            {
                switch (button)
            ...
            // treat touch as pointer press
            if (Touchscreen.current != null && GetTouchCount() > 0)
            {
                return true;
            }
        ...
        protected bool WasPointerPressedThisFrame(MouseButton button)
        {
            if (useNewInputSystem && Mouse.current != null)
            {
                switch (button)
        ...
        protected int GetTouchCount()
        {
            if (useNewInputSystem && Touchscreen.current != null)
            {
                ...
                foreach (var t in Touchscreen.current.touches)
                {
                    if (t.press != null && t.press.isPressed) count++;
                }
        ...
        protected float GetCustomAxis(string axisName)
        {
            if (string.IsNullOrEmpty(axisName)) return 0f;

            // No generic mapping available here — fall back to legacy axis by name
        ...
        protected bool GetButton(string buttonName)
        {
            ...
            // keep legacy behavior for named buttons/triggers
